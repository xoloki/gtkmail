...
URIHandlerDialog::URIHandlerDialog(Address addr)
{
...
void URIHandlerPreflet::load() {
    for(URIHandler::iterator i = URIHandler::global.begin(); i != URIHandler::global.end(); i++) {
...
void URIHandlerPreflet::on_new() {
	...
	try {
	    ...
	    URIHandler::global.push_back(addr);
...
void URIHandlerPreflet::on_edit() {
    ...
    URIHandler::iterator i = URIHandler::global.find(key);
    if(i == URIHandler::global.end()) return;
    
    URIHandlerDialog dlg(*i);
